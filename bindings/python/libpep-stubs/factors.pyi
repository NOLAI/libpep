# Auto-generated by generate_stubs.py — do not edit by hand.

class AttributeRekeyFactor:
    """A factor used to rekey attributes between sessions."""
    def __eq__(self, value): ...
    def __init__(self, scalar): ...
    def scalar(self): ...

class AttributeRekeyInfo:
    """The information required to rekey attributes from one session to another."""
    def __eq__(self, value): ...
    def __init__(self, session_from, session_to, encryption_secret, context=None): ...
    @property
    def k(self): ...
    def rekey_public_key(self, before): ...
    def reverse(self): ...

class EncryptionSecret:
    """Encryption secret used to derive rekey factors from encryption contexts."""
    def __eq__(self, value): ...
    def __init__(self, data): ...
    @staticmethod
    def from(data): ...

class PseudonymRekeyFactor:
    """A factor used to rekey pseudonyms between sessions."""
    def __eq__(self, value): ...
    def __init__(self, scalar): ...
    def scalar(self): ...

class PseudonymRekeyInfo:
    """The information required to rekey pseudonyms from one session to another."""
    def __eq__(self, value): ...
    def __init__(self, session_from, session_to, encryption_secret, context=None): ...
    @property
    def k(self): ...
    def rekey_public_key(self, before): ...
    def reverse(self): ...

class PseudonymizationInfo:
    """The information required to pseudonymize from one domain and session to another."""
    def __eq__(self, value): ...
    def __init__(self, domain_from, domain_to, session_from, session_to, pseudonymization_secret, encryption_secret, context=None): ...
    @property
    def k(self): ...
    @property
    def rekey_info(self): ...
    def rekey_public_key(self, before): ...
    def reverse(self): ...
    @property
    def s(self): ...

class PseudonymizationSecret:
    """Pseudonymization secret used to derive reshuffle factors from pseudonymization domains."""
    def __eq__(self, value): ...
    def __init__(self, data): ...
    @staticmethod
    def from(data): ...

class RerandomizeFactor:
    """A factor used to rerandomize an ElGamal ciphertext."""
    def __eq__(self, value): ...
    def __init__(self, scalar): ...
    def scalar(self): ...

class ReshuffleFactor:
    """A factor used to reshuffle an ElGamal ciphertext."""
    def __eq__(self, value): ...
    def __init__(self, scalar): ...
    def scalar(self): ...

class TranscryptionInfo:
    """The information required to transcrypt from one domain and session to another."""
    def __eq__(self, value): ...
    def __init__(self, domain_from, domain_to, session_from, session_to, pseudonymization_secret, encryption_secret, context=None): ...
    @property
    def attribute(self): ...
    @property
    def pseudonym(self): ...
    def rekey_public_keys(self, before): ...
    def reverse(self): ...

class VerifiablePseudonymizationCommitment:
    """Pseudonymization factor commitment for a single transition (Python)."""
    def __eq__(self, value): ...
    def __init__(self, *args, **kwargs): ...
    @staticmethod
    def from_json(json): ...
    def to_json(self): ...

class VerifiableRekeyCommitment:
    """Rekey factor commitment for a single transition (Python)."""
    def __eq__(self, value): ...
    def __init__(self, *args, **kwargs): ...
    @staticmethod
    def from_json(json): ...
    def to_json(self): ...

class VerifiableTranscryptionCommitment:
    """Combined transcryption commitments — pseudonymization (reshuffle + rekey)"""
    def __eq__(self, value): ...
    def __init__(self, *args, **kwargs): ...
    @staticmethod
    def from_json(json): ...
    def to_json(self): ...

def make_attribute_rekey_factor(secret, session, context=None):
    """Derive the attribute rekey factor of a session from an encryption secret within the protocol"""
    ...

def make_pseudonym_rekey_factor(secret, session, context=None):
    """Derive the pseudonym rekey factor of a session from an encryption secret within the protocol"""
    ...

def make_pseudonymisation_factor(secret, domain, context=None):
    """Derive the reshuffle factor of a domain from a pseudonymization secret within the protocol"""
    ...
