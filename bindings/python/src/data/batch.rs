...
impl PyRecordBatch {
    ...
    fn new(
        ...
        session_keys: &crate::keys::distribution::shares::PySessionPublicKeys,
    ) -> PyResult<Self> {
        let items: Vec<EncryptedRecord> = items.into_iter().map(|r| r.0).collect();
        let keys: libpep::keys::SessionPublicKeys = (*session_keys).into();
    ...
    /// `(batch-pk, !elgamal3)`).
    #[cfg(all(not(feature = "elgamal3"), feature = "batch-pk"))]
    fn public_key(&self) -> crate::keys::distribution::shares::PySessionPublicKeys {
        rust_session_public_keys_to_py(&self.inner.public_key)
    }
...
#[cfg(all(not(feature = "elgamal3"), feature = "batch-pk"))]
fn rust_session_public_keys_to_py(
    keys: &libpep::keys::SessionPublicKeys,
) -> crate::keys::distribution::shares::PySessionPublicKeys {
    ...
    crate::keys::distribution::shares::PySessionPublicKeys {
        pseudonym: PyPseudonymSessionPublicKey(PyGroupElement(*libpep::keys::PublicKey::value(
            &keys.pseudonym,
        ))),
        attribute: PyAttributeSessionPublicKey(PyGroupElement(*libpep::keys::PublicKey::value(
            &keys.attribute,
        ))),
    }
}
