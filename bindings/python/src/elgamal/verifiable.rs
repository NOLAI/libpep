...
fn py_verifiable_rerandomize(
    ...
    let mut rng = rand::rng();
    let (result, proof) =
        libpep::elgamal::verifiable::verifiable_rerandomize(&original.0, &r.0, &mut rng);
    (
...
fn py_verifiable_rerandomize(
    ...
    let mut rng = rand::rng();
    let (result, proof) =
        libpep::elgamal::verifiable::verifiable_rerandomize(&original.0, &gy.0, &r.0, &mut rng);
    (
        PyElGamal::from(result),
        PyVerifiableRerandomize { inner: proof },
    )
...
impl PyVerifiableRSKInner {
    fn verify(
        &self,
        original: &PyElGamal,
        gt: &crate::elgamal::arithmetic::PyGroupElement,
...
#[cfg(feature = "batch")]
fn into_elgamal_vec(cts: &[PyElGamal]) -> Vec<crate::elgamal::ElGamal> {
    cts.iter().map(|c| c.0).collect()
}

#[cfg(feature = "batch")]
fn into_py_elgamal_vec(cts: Vec<crate::elgamal::ElGamal>) -> Vec<PyElGamal> {
...
fn py_verifiable_rerandomize_batch(
    ...
    let cts = into_elgamal_vec(&originals);
    let (results, proof) =
        libpep::elgamal::verifiable::verifiable_rerandomize_batch(&cts, &gy, &mut rng);
    Ok((
...
fn py_verifiable_rerandomize_batch(
    ...
    let cts = into_elgamal_vec(&originals);
    let (results, proof) =
        libpep::elgamal::verifiable::verifiable_rerandomize_batch(&cts, &gy.0, &mut rng);
    (
        into_py_elgamal_vec(results),
        PyVerifiableRerandomizeBatch { inner: proof },
    )
