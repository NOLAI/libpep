...
#[cfg(feature = "batch")]
fn into_elgamal_vec(cts: &[WASMElGamal]) -> Vec<crate::elgamal::ElGamal> {
    cts.iter().map(|c| **c).collect()
}

#[cfg(feature = "batch")]
fn into_wasm_elgamal_vec(cts: Vec<crate::elgamal::ElGamal>) -> Vec<WASMElGamal> {
...
pub fn wasm_verifiable_rerandomize(
    ...
    let mut rng = rand::rng();
    let (result, proof) =
        libpep::elgamal::verifiable::verifiable_rerandomize(original, r, &mut rng);
    WASMVerifiableRerandomizeResult {
...
pub fn wasm_verifiable_rerandomize(
    ...
    let mut rng = rand::rng();
    let (result, proof) =
        libpep::elgamal::verifiable::verifiable_rerandomize(original, gy, r, &mut rng);
    WASMVerifiableRerandomizeResult {
        result: WASMElGamal::from(result),
        proof: WASMVerifiableRerandomize(proof),
    }
...
impl WASMVerifiableRSKInner {
    pub fn verify(
        &self,
        original: &WASMElGamal,
        gt: &crate::elgamal::arithmetic::group_elements::WASMGroupElement,
