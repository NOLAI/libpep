...
#[cfg(all(feature = "batch-pk", not(feature = "elgamal3")))]
impl SessionKeys {
    /// Convert this session key bundle to its rekeyed counterpart, applying the
    /// pseudonym rekey factor to the pseudonym half and the attribute rekey
    /// factor to the attribute half. Secret keys are left unchanged, as they are
    /// not part of the public-key material that travels with a batch.
    pub fn convert(
        &self,
