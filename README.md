...
| `scalar`, `point` | `random`, `invert`, `mul`, `from-hash`; `random`, `base`, `mul`, `from-hash`, `hash-to-group` |
...
transcrypted=$(peppy pseudonym transcrypt \
    ...
    --key "$(jq -r .pseudonym_public_key session-a.json)" "$encrypted")
...
| `elgamal` | The ElGamal ciphertext, the PEP primitives (`rekey`, `reshuffle`, `rerandomize` and their combinations) in `elgamal::primitives`, the Ristretto scalar and group element arithmetic in `elgamal::arithmetic`, `expand_message_xmd`, `hash_to_group` and `hash_to_scalar` in `elgamal::arithmetic::hashing`, and the verifiable variants of the primitives with their proofs in `elgamal::verifiable` |
