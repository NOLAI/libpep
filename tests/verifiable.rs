...
fn test_verifiable_record_transcryption() {
    ...
    #[cfg(not(feature = "elgamal3"))]
    let proof =
        transcryptor.verifiable_transcrypt(&enc_record, &info, &session_a_keys.public_keys(), rng);
...
fn tampered_proof_rejected_record_transcryption() {
    ...
    #[cfg(not(feature = "elgamal3"))]
    let donor_proof =
        transcryptor.verifiable_transcrypt(&donor_enc, &info, &session_a_keys.public_keys(), rng);
    ...
    #[cfg(not(feature = "elgamal3"))]
    let result = verifier.verify_transcryption::<_>(
        &enc_record,
        &tampered,
        &session_a_keys.public_keys(),
        &commitments,
...
fn wrong_recipient_pk_rejected_pseudonymization_batch() {
    ...
    let other_session_pk = PseudonymSessionPublicKey::from_point(*other_pk);
...
#[cfg(all(feature = "batch", any(feature = "elgamal3", feature = "batch-pk")))]
#[test]
