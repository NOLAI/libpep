...
fn test_proved() {
    ...
    let pseudonymized = proved_pseudonymize(&enc_pseudo, &pseudo_context1, &pseudo_context2, &enc_context1, &enc_context2, &pseudo_secret, &enc_secret);

    let pseudonymized_dec = proved_decrypt_pseudonym(&pseudonymized, &enc_pseudo, &session2_secret, &pseudo_verifiers1, &pseudo_verifiers2, &rekey_verifiers1, &rekey_verifiers2);

    assert!(pseudonymized_dec.is_some());
    assert_ne!(pseudo, pseudonymized_dec.unwrap());

    let enc_pseudo_2 = pseudonymized.reconstruct(&enc_pseudo, &pseudo_verifiers1, &pseudo_verifiers2, &rekey_verifiers1, &rekey_verifiers2);
    let dec_pseudo_2 = decrypt_pseudonym(&enc_pseudo_2.unwrap(), &session2_secret);
    assert_eq!(pseudonymized_dec.unwrap(), dec_pseudo_2);

    let non_proved = pseudonymize(&enc_pseudo, &pseudo_context1, &pseudo_context2, &enc_context1, &enc_context2, &pseudo_secret, &enc_secret);
    let non_proved_dec = decrypt_pseudonym(&non_proved, &session2_secret);
    assert_eq!(pseudonymized_dec.unwrap(), non_proved_dec);

    let rev_pseudonymized = proved_re_pseudonymize(&pseudonymized, &enc_pseudo, &pseudo_context2, &pseudo_context1, &enc_context2, &enc_context1, &pseudo_secret, &enc_secret, &pseudo_verifiers1, &pseudo_verifiers2, &rekey_verifiers1, &rekey_verifiers2);
    ...
    assert_eq!(pseudo, rev_pseudonymized_dec.unwrap());
}